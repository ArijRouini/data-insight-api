...
MAX_LLM_ITEMS = 24  # soft guardrail for huge datasets
...
- "summary": a markdown string of 3–5 bullet points (each line starts with "- ").

Rules:
- Use ONLY facts present in Context Statistics or implied by the chart type (no speculation).
- Prefer exact numbers: mean/median/min/max/std, counts, % missing, r, IQR bounds, etc.
- If a field name includes "id", "code", or "index", state it is an identifier with limited analytic value.
- No recommendations, no actions, no first-person. Keep bullets ≤ 18 words each. Dashboard tone.
...
Write 3–5 concise quantitative executive bullets.
Constraints:
...
Write a precise, verification-friendly narrative (2–4 short paragraphs). Requirements:
- No greeting; no first-person; no recommendations.
- Use ONLY facts from Context (no invented numbers).
- Paragraph 1: quantitative overview — dataset size, variables, key totals/ranges.
- Paragraph 2: relationships/anomalies with concrete metrics (e.g., r, outlier counts via 1.5×IQR, min/max, % missing).
- Paragraph 3 (optional): data completeness/quality and readiness for further analysis.
...
class ApiResponse(BaseModel):
    ...
    analysis_kit: Optional[dict] = None                  # { categories: [...] }
    charts: Optional[dict] = None                        # { chart_id: figure }
...
async def inspect_data(file: UploadFile = File(...)):
    ...
    if pairs:
        top = max(pairs, key=lambda d: abs(d["r"]))
